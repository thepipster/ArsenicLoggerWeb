...
exports.registerUser = function(req, res, next){
    ...
    // Make sure this username is not in use already
    User.loadFromUsername(username, function(err, user){
        if (user){
            res.send({result:"fail", error: "Sorry, this username is taken"});
        }
    });
            ...
            User.create(accountObj.id, username, 'admin', 'approved', function(err, userObj){
                ...
                userObj.save(function(err, savedUser){
                    if (err) {
                        ...
                        var Messaging = require('../utils/Messaging.js');
                        Messaging.sendNewSignNotification('mike@arsenicsoup.com', 'Mike', username);
