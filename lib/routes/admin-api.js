...
exports.getLogs = function(req, res){
    ...
    var lastSync = req.params.lastSync || new Date(0);

    //if (!page) page = 1;
    //if (!pageSize) pageSize = 20;
    if (!tag) tag = "";

    var query = {'modified': {$gte: lastSync}};
    ...
    if (req.user) {
        ...
        LogModel.count(query, function(err, size){
            ...
            if (!err){
                ...
                LogModel.find(query).sort({'modified': 'descending'}).skip(from).limit(to).exec(function(err, logs){
                    if (!err){
